...
df_ewallet = df[df['biller_name'].isin(ewallet_services)].copy()

# Proses waktu transaksi
df_ewallet['received_time'] = pd.to_datetime(df_ewallet['received_time'], errors='coerce')
df_ewallet['day_of_week'] = df_ewallet['received_time'].dt.dayofweek  # Senin=0, Minggu=6

# Encode operator_code jadi angka (opsional)
df_ewallet['operator_encoded'] = LabelEncoder().fit_transform(df_ewallet['operator_code'].astype(str))

features = ['trx_value', 'day_of_week']
X = df_ewallet[features].dropna()
...
df_cluster = df_ewallet.loc[X.index].copy()
...
plt.title('Clustering Layanan E-wallet Berdasarkan Nilai Transaksi dan Hasil Transaksi')
